...
class WPZOOM_Rating_DB {
	 ...
	 * @var array $fields
	 */
	private static $fields = array( 'id', 'recipe_id', 'user_id', 'comment_id', 'post_id', 'rating', 'rate_date', 'update_date', 'ip', 'approved', 'author_name', 'author_email', 'review_text' );

	/**
	 * Memoised result of the table-existence check.
	 *
	 * @access private
	 * @var bool|null $table_exists
	 */
	private static $table_exists = null;

	/**
	 * Whether the rating table exists.
	 *
	 * Memoised because the raw SHOW TABLES check used to run on every rating
	 * query; that adds up quickly on archive pages rendering many cards.
	 * Reset to null whenever the table may have just been created.
	 *
	 * @since 3.5.0
	 * @return bool
	 */
	private static function table_exists() {
		if ( null === self::$table_exists ) {
			...
			self::$table_exists = (bool) $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', self::get_table_name() ) );
		}
	...
	public static function create_or_update_database( $from ) {
		...
		// The table may have just been created - invalidate the memoised check.
	...
	public static function add_or_update_rating( $rating_data ) {
		...
		if ( 0 < $rating['rating'] && 5 >= $rating['rating'] ) {
			...
			// Built through $wpdb->prepare() because get_ratings() interpolates the
			// resulting string straight into its query.
			if ( $rating['id'] ) {
				$where = $wpdb->prepare( 'id = %d AND ip = %s', $rating['id'], $rating['ip'] );
			} elseif ( $rating['recipe_id'] ) {
				if ( $rating['user_id'] ) {
					$where = $wpdb->prepare( 'recipe_id = %d AND user_id = %d', $rating['recipe_id'], $rating['user_id'] );
				} elseif ( $rating['ip'] ) {
					$where = $wpdb->prepare( 'recipe_id = %d AND ip = %s', $rating['recipe_id'], $rating['ip'] );
				} else {
					$where = $wpdb->prepare( 'recipe_id = %d AND rate_date = %s AND ip = %s', $rating['recipe_id'], $rating['rate_date'], $rating['ip'] );
				}
			} elseif ( $rating['comment_id'] ) {
				$where = $wpdb->prepare( 'comment_id = %d', $rating['comment_id'] );
			}
			...
			if ( $where ) {
				...
				if ( ! $rating['recipe_id'] && class_exists( 'WPZOOM_Comment_Rating' ) ) {
	...
	public static function delete_ratings( $ids ) {
		...
		if ( is_array( $ids ) ) {
			...
			$wpdb->query( 'DELETE FROM ' . $table_name . ' WHERE id = ' . absint( $ids ) );
		}
	}
