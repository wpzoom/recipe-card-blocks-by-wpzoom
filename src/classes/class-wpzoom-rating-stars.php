...
class WPZOOM_Rating_Stars {
	...
	/**
	 * Get the effective user-ratings mode for the Lite plugin.
	 *
	 * The confirmation modal is a PRO feature, so a stored 'modal' value
	 * (e.g. after downgrading from PRO) falls back to instant rating.
	 *
	 * @since 3.5.0
	 * @return string 'instant' or 'jump_to_comments'.
	 */
	public static function get_rating_mode() {
		$mode = WPZOOM_Settings::get( 'wpzoom_rcb_settings_user_ratings_mode' ) ?: 'instant';

		if ( ! in_array( $mode, array( 'instant', 'jump_to_comments' ), true ) ) {
			$mode = 'instant';
		}
	...
	/**
	 * Check if a rating submission is spam using Akismet.
	 *
	 * @since 3.3.0
	 * @param array  $rating        The rating data.
	 * @param string $comment_text  Optional comment text.
	 * @return bool True if the submission is spam, false otherwise.
	 */
	public static function is_spam( $rating, $comment_text = '' ) {
		// Check if Akismet is available and spam protection is enabled
		$spam_protection = WPZOOM_Settings::get( 'wpzoom_rcb_settings_rating_spam_protection' );
		...
		// Prepare the data for Akismet
		$query_string = http_build_query(
			array(
				'comment_type'         => 'recipe-rating',
				'comment_author'       => ! empty( $rating['author_name'] ) ? $rating['author_name'] : '',
				'comment_author_email' => ! empty( $rating['author_email'] ) ? $rating['author_email'] : '',
				...
				'permalink'            => get_permalink( $rating['recipe_id'] ),
				'is_test'              => false,
			)
		);
	...
	public static function display_rating_modal_notice() {
		...
		if ( 'edit-wpzoom_rcb' !== $current_screen->id ) {
			return;
		}
	...
	public static function set_rating() {
		...
		// Check for spam using Akismet (if enabled and available)
		// Only check when there's user-submitted content (modal mode with name/email/comment)
		$has_user_content = ! empty( $rating['author_name'] ) || ! empty( $rating['author_email'] ) || ! empty( $rating_comment );
		if ( $has_user_content && self::is_spam( $rating, $rating_comment ) ) {
			$response = array(
				'status'  => '403',
				'message' => __( 'Your rating could not be submitted. Please try again later.', 'recipe-card-blocks-by-wpzoom' ),
			);
			wp_send_json_error( $response );
		}
	...
	public static function get_rating_star_svg( $type ) {
		...
		if ( 'full' === $type || 'empty' === $type ) {
			return self::with_default_star_svg_size( WPZOOM_Comment_Rating::get_rating_star_svg( $type ) );
		}
		return '';
	}
