...
import { Button, Popover, TextControl, CheckboxControl } from '@wordpress/components';
...
const ButtonBox = ( props ) => {
    ...
    let defaultRecipePrompt = '',
        defaultImagePrompt = '',
        defaultNutritionPrompt = '',
        defaultCustomImagePrompt = true;
    if ( prompts ) {
        if ( prompts.recipe ) {
            defaultRecipePrompt = prompts.recipe;
        }
        if ( prompts.image ) {
            defaultImagePrompt = prompts.image;
        }
        if ( prompts.nutrition ) {
            defaultNutritionPrompt = prompts.nutrition;
        }
        if ( defaultRecipePrompt !== defaultImagePrompt ) {
            defaultCustomImagePrompt = false;
        }
    }

    const { setRecipeData, setMessageToAI } = useRecipeDataActions();
    const [ isVisible, setIsVisible ] = useState( false );
    const [ inputValue, setInputValue ] = useState( defaultRecipePrompt );
    const [ customImagePrompt, setCustomImagePrompt ] = useState( defaultCustomImagePrompt );
    const [ generateRecipeImage, setGenerateRecipeImage ] = useState( true );
    const [ imagePrompt, setImagePrompt ] = useState( defaultImagePrompt );
    const [ nutritionPrompt, setNutritionPrompt ] = useState( defaultNutritionPrompt );
    ...
    useEffect(() => {
        const placeholderElement = document.querySelector('.wp-block-wpzoom-recipe-card-block-recipe-card');
        if (placeholderElement) {
            if (isImageLoading) {
                placeholderElement.classList.add('image-loader');
            } else {
                placeholderElement.classList.remove('image-loader');
            }
        }
    }, [isImageLoading]);
    ...
    const handleSubmit = async() => {
        try {
            ...
            const response = await fetch( licenseData.endpoint_url + 'wp-json/wp-zoom-openai/v1/purchase', {
                ...
                body: JSON.stringify( {
                    message: {
                        recipe: recipePromptPrepend + inputValue + recipePromptAppend,
            ...
            if ( responseData.error === 'Insufficient credits' ) {
                ...
                setRecipeData( chatData, null );
                setSuccess( true );
                setIsLoading( false );

                if( generateRecipeImage ) {
                    if ( imgData.error ) {
                        setSuccess( false );
                        setError( 'Error in AI Response,\nTry using a different prompt.' );
                        return false;
    ...
    const handleAddManuallyClick = () => {
        ...
        setIsGenerateAIVisible( true ); // Show "Generate AI" button
    };

    const handleclosed = () => {
        setError( false ); // Close the popover
        setErrorLogin( false ); // Close the popover
        setCreditserror( false ); // Close the popover
        setIsAddManuallyVisible( false ); // Hide "Add Manually" button
        setIsVisible( false ); // Show "Generate AI" button
    };
    ...
    const toggleAiPopup = () => {
        ...
        setIsAddManuallyVisible( false ); // Hide "Add Manually" button
        setIsVisible( true ); 
        isVisible( true ); // Show "Generate AI" button
    };

    // List of suggestions
    const suggestions = [
        __( 'Classic Italian pasta', 'recipe-card-blocks-by-wpzoom' ),
        __( 'Low-carb dinner recipe', 'recipe-card-blocks-by-wpzoom' ),
        __( 'Beginner-friendly dessert recipe', 'recipe-card-blocks-by-wpzoom' ),
        __( 'I want a recipe with a twist. Surprise me!', 'recipe-card-blocks-by-wpzoom' ),
        __( 'Generate a festive holiday dessert idea.', 'recipe-card-blocks-by-wpzoom' ),
    ];
    ...
    return (
        <div className="ai-div">
            { /* "Generate AI" button */ }
            { isGenerateAIVisible && (
            <Button className="ai-button" onClick={ toggleVisible }>
            ...
            >
                ...
                <span className="btn-text">{ __( 'Generate with AI', 'recipe-card-blocks-by-wpzoom' ) }</span>
            </Button>
      ) }
            { /* "Add Manually" button (initially hidden) */ }
            { isAddManuallyVisible && (
            <Button className="manual-button" onClick={ handleAddManuallyClick }>
                { __( 'Add manually', 'recipe-card-blocks-by-wpzoom' ) }
            </Button>
      ) }
            { /* Popover */ }
            { isVisible && (
            <Popover className="popup-overlay" position="center center">
                <div className="popup-content">
                  ...
                  >
                      ...
                      />
                            ...
                            <TextControl
                    placeholder={ __( 'Enter recipe title here' ) }
                    value={ inputValue }
                ...
                <CheckboxControl
                  label={ __( 'Generate Recipe Image with AI' ) }
                  help={ __( 'This step might slow down the process (~30 seconds)' ) }
                  checked={ generateRecipeImage }
                  className="use-recipe-image-prompt"
                  onChange={ ( newValue ) => {
                    setGenerateRecipeImage( newValue );
                  } }
              />{ generateRecipeImage && (
                <CheckboxControl
                  label={ __( 'Use the same prompt for image' ) }
                  checked={ customImagePrompt }
                  className="use-recipe-image-prompt"
                  onChange={ ( newValue ) => {
                    setCustomImagePrompt( newValue );
                  } }
              /> ) }
                        { ! customImagePrompt && (
                        <div className="svg-input">
                  ...
                  >
                      ...
                      />
                            ...
                            <TextControl
                    placeholder={ __( 'Image Prompt' ) }
                    value={ imagePrompt }
                    className="recipe-image-prompt-field"
                    onChange={ ( newValue ) => {
                  ...
                  >
                    ...
                    <div className="Content-suggestions">
                        ...
                        { suggestions.map( ( suggestion, index ) => (
                            <li key={ index } onClick={ () => handleChange( suggestion ) }>
                                { suggestion }
                            </li>
                        ) ) }
                        </ul>
				...
				<Popover className="popup-overlay" position="center center">
					<div className="popup-content popup-content-error">
						...
						<div className="Content-suggestions">
							...
							<h4>{ __( 'We\'re sorry, an unexpected error has occurred.', 'recipe-card-blocks-by-wpzoom' ) }</h4>
							<p>{ __( 'Please connect your Recipe Card Blocks account to your website', 'recipe-card-blocks-by-wpzoom' ) }</p>
							<a className="components-button buyMore" href={`${siteURL}/wp-admin/admin.php?page=admin-license`}>{ __( 'Connect your account', 'recipe-card-blocks-by-wpzoom' ) }</a>
						</div>
					</div>
				</Popover>
			) }
                ...
                <Popover className="popup-overlay" position="center center">
                    <div className="popup-content popup-content-error">
                        ...
                        <div className="Content-suggestions">
                            ...
                            <h4>{ __( 'We\'re sorry, an unexpected error has occurred.', 'recipe-card-blocks-by-wpzoom' ) }</h4>
                            <p>{ __( 'Refresh the page, this might clear the error and allow you to continue.', 'recipe-card-blocks-by-wpzoom' ) }</p>
                            <Button className="try-again" onClick={ toggleAiPopup }>
                                <span className="btn-text">{ __( 'Try again', 'recipe-card-blocks-by-wpzoom' ) }</span>
                            </Button>
                        </div>
                    </div>
                </Popover>
            ...
            <Popover className="popup-overlay" position="center center">
                <div className="popup-content popup-content-error">
                    ...
                    <div className="Content-suggestions ai-credits-error">
                        ...
                        <h4>{ __( 'There are no AI Credits left', 'recipe-card-blocks-by-wpzoom' ) }</h4>
                        <p className="ai-p">{ __( 'Refill your balance for uninterrupted access to AI Recipe Generator functionalities.', 'recipe-card-blocks-by-wpzoom' ) }</p>
                        <span className="ai-error"><a href="https://recipecard.io/ai-recipe-generator/" target="_blank">{ __( 'Learn more about AI Recipe Generator', 'recipe-card-blocks-by-wpzoom' ) }</a></span>
                        <Button className="try-again ai-error" target="_blank" href={`${storeURL}account/ai-credits/`}>
                            <span className="btn-text">{ __( 'Buy more AI credits', 'recipe-card-blocks-by-wpzoom' ) }</span>
                        </Button>
                    </div>
                </div>
            </Popover>
